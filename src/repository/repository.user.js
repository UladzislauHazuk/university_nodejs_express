...
async function createUsersDB(name, surname, birth, city, age) {
    ...
    try {
        ...
        const sql3 = 'SELECT users.name, users.surname, users_info.birth, users_info.city, users_info.age FROM users JOIN users_info ON users_info.id = users.info_id WHERE users.info_id = $1';
...
async function updateUsersDB(name, surname, birth, city, age, info_id) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
...
async function patchUsersDB(info_id, dataFromClient) {
    ...
    try {
        ...
        const sql = 'SELECT * FROM users JOIN users_info ON users.info_id = users_info.id WHERE info_id = $1';
        ...
        const sql4 = 'SELECT * FROM users JOIN users_info ON users.info_id = users_info.id WHERE info_id = $1';
